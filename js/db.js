// ============================================================
// db.js — All Supabase database calls with proper error handling
// ============================================================
...
async function getMemberById(lionId){
  try {
    const { data, error } = await DB.from('members').select('*').eq('lion_id', lionId).single();
...
async function heartbeat(lionId, name){
  await DB.from('online_members').upsert(
    { lion_id: lionId, name, last_seen: new Date().toISOString() },
    { onConflict: 'lion_id' }
  );
}
...
async function getOnlineMembers(){
  try {
    ...
    const { data } = await DB.from('online_members').select('*').gte('last_seen', cutoff);
    return data || [];
  } catch(e){ return []; }
}
